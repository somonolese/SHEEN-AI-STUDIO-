...
export default function SimilarAppsScreen() {
  ...
  const baseApps = useMemo(() => {
    if (!app) return [];
    return allApps.filter((a) => a.id !== app.id && (a.categoryId === app.categoryId || a.source === app.source));
  }, [allApps, app]);
  ...
  const renderHeader = useCallback(() => (
    <>
      ...
      >
        <LinearGradient
          colors={[app.color || colors.primary, `${app.color || colors.primary}CC`]}
          start={{ x: 0, y: 0 }}
          end={{ x: 1, y: 1 }}
          style={[styles.heroCard, { borderColor: colors.border }]}
        >
          <View style={styles.heroIconWrap}>
            <MaterialCommunityIcons name="view-grid-plus-outline" size={40} color="#fff" />
          </View>
          <ThemedText style={styles.heroName}>Similar to {app.name}</ThemedText>
  ...
  ), [app, colors, query, sort, apps.length]);

  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      <Stack.Screen options={{ title: 'Similar Apps' }} />
